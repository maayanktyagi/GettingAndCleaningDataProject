...
sensor_labels <- rbind(sensor_labels_row, c(563,"A"))[,2]

names(sensor_data) <- sensor_labels

mean_data <- sensor_data[,grepl("mean",names(sensor_data))]

std_data <- sensor_data[,grepl("mean|std|Subject|ActivityId", names(sensor_data))]


merged_data <- join(std_data,activity_labels , by = "ActivityId", match = "first") 
